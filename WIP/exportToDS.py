...
def rhArcCurveToMSArc(item):
    ...
    return MSArc(startPoint, midPoint, endPoint)
...
#PolyCurve
            ...
            for i in range(0, segmentCount):
                ...
                if type(curve) == rc.Geometry.LineCurve:
                    msSubCurves.append(rhLineCurveToMSLine(curve))
                elif type(curve) == rc.Geometry.PolyCurve and curve.IsPolyLine() == True:
                    msSubCurves.append(rhPolyLineToMSPolyLine(curve.TryGetPolyline()[1]))
                elif type(curve) == rc.Geometry.PolylineCurve and curve.IsPolyline() == True:
                    msSubCurves.append(curve.TryGetPolyline()[1])
            ...
            geometryOut.append(msPolyCurve)
