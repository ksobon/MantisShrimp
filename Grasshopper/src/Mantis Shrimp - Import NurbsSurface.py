...
"""
This component reads NurbsSurfaces exported from Dynamo

    Args:
        _filePath: path to file to read
        _import: boolean True or False that will either allow or prevent data \
            from being imported
"""
ghenv.Component.Name = "Mantis Shrimp - NurbsSurface"
ghenv.Component.NickName = 'DSNurbsSurface'
...
def toRHObject(item):
    if type(item) == ms.MSNurbsSurface:
        return item.toRHNurbsSurface(sc.doc.ModelUnitSystem.ToString())
...
if _import:
    ...
    outNurbsSurface = list_to_tree(dataList)
